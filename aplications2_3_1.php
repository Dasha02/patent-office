...
if (isset($_POST['begin_verification']))
{
    $aplication_id = sanitizeString($_POST['aplication_id']);
    $user_id_verif = sanitizeString($_POST['zanyato']);
    ...
    // FOR UPDATE SKIP LOCKED
    {
      ...
      $num_rows = $result->rowCount();
        if ($num_rows!=0)
        {
            ...
            if ($user_verificator_id != 1 and $user_verificator_id != $user_id_verif )
              { 
              ...
              {
                if ($user_verificator_id == "1")
                  {
                    $pdo->query ("UPDATE aplication_form SET user_verificator_id=$user_id_verif WHERE aplication_id=$aplication_id");
                  }
                $wr = 1;  
                $pdo->commit(); 
              }        
...
if (isset($_POST['submit']))
{
    ...
    {
        $verification  = sanitizeString($_POST['verification']);
       ...
       // $pdo->commit();  
        ...
        { 
            ...
            if ($num_rows != 0)
            {
                ...
                if ($last_status_id != $present_status_id AND $user_id == $user_verificator_id)
                {
                    $err= "";
    ...
    {
        echo "<script>alert('ВЕРИФИКАЦИЯ НЕ ЗАВЕРШЕНА!!! ВЕРНИТЕСЬ ПОЗЖЕ К ЗАДАЧЕ ДЛЯ ЗАВЕРШЕНИЯ ПРОЦЕССА!!!');</script>";
        header("Refresh: 0; url= aplications2.php");
    }   
}
...
echo <<< _begin
    ...
    <a align="center">ВЕРЕФИКАЦИЯ ДОКУМЕНТОВ ПО ЗАЯВКЕ</a>
   </div>
    <div id='$id' >
        ...
        <div class='container'>
              ...
              <form action='aplications2_3_1.php' method='post'>

              <fieldset>
              <legend>По результатам анализа пакета документов примите решение:</legend> <br>
              <input type='radio' name='verification' value='1'>ДОКУМЕНТЫ ВЕРЕФИЦИРОВАНЫ - ПОЛНЫЙ КОМПЛЕКТ<br><br>
              <input type='radio' name='verification' value='0'>ДОКУМЕНТЫ НЕ ВЕРИФИЦИРОВАНЫ - ОТКАЗ В РАССМОТРЕНИИ ЗАЯВКИ<br><br>
