...
if (isset($_POST['begin_znach']))
{
    $aplication_id       = sanitizeString($_POST['aplication_id']);
    $user_id_expert_znak = sanitizeString($_POST['zanyato']);
    ...
    try
    {
      ...
      $num_rows = $result->rowCount();
        if ($num_rows!=0)
        {
            ...
            if ($user_expert_znak_id != 1 and $user_expert_znak_id != $user_id_expert_znak)
              { 
              ...
              {
                if ($user_expert_znak_id == "1")
                  {
                    $pdo->query ("UPDATE aplication_form SET user_expert_znak_id=$user_id_expert_znak WHERE aplication_id=$aplication_id");
                  }
                $wr = 1;  
                $pdo->commit(); 
              }        
...
if (isset($_POST['submit']))
{
    ...
    {
        $znach         = sanitizeString($_POST['znach']);
        $aplication_id = sanitizeString($_POST['ap_id']);
        $user_id_ex_zn = sanitizeString($_POST['zanyato']);
       ...
       // $pdo->commit();  
        ...
        { 
            ...
            if ($num_rows != 0)
            {
                ...
                if ($last_status_id != $present_status_id AND $user_id == $user_expert_znak_id)
                {
                    $err= "";
...
echo <<< _begin
    ...
    <div id='$id' >
        <div class='container'>
              ...
              <form action='aplications2_2_1.php' method='post'>

              <fieldset>
              <legend>По результатам анализа пакета документов примите решение:</legend> <br>
              <input type='radio' name='znach' value='1'>ДОКУМЕНТЫ ПРОВЕРЕНЫ - ЗНАЧИМОСТЬ ПОДТВЕРЖДАЕТСЯ<br><br>
              <input type='radio' name='znach' value='0'>ДОКУМЕНТЫ ПРОВЕРЕНЫ - ЗНАЧИМОСТЬ НЕ ПОДТВЕРЖДАЕТСЯ - ЗАЯВКА ОТКЛОНЯЕТСЯ<br><br>
