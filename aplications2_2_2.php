...
if (isset($_POST['begin_plagiat']))
{
    $aplication_id       = sanitizeString($_POST['aplication_id']);
    $user_id_expert_plag = sanitizeString($_POST['zanyato']);
    ...
    try
    {
      ...
      $num_rows = $result->rowCount();
        if ($num_rows!=0)
        {
            ...
            if ($user_expert_plag_id != 1 and $user_expert_plag_id != $user_id_expert_plag)
              { 
              ...
              {
                if ($user_expert_plag_id == "1")
                  {
                    $pdo->query ("UPDATE aplication_form SET user_expert_plag_id=$user_id_expert_plag WHERE aplication_id=$aplication_id");
                  }
                $wr = 1;  
                $pdo->commit(); 
              }        
...
if (isset($_POST['submit']))
{
    ...
    {
        $plagiat       = sanitizeString($_POST['plagiat']);
        $aplication_id = sanitizeString($_POST['ap_id']);
        $user_id_ex_pl = sanitizeString($_POST['zanyato']);
       ...
       // $pdo->commit();  
        ...
        { 
            ...
            if ($num_rows != 0)
            {
                ...
                if ($last_status_id != $present_status_id AND $user_id == $user_expert_plag_id)
                {
                    $err= "";
            ...
            echo "<script>alert('Изменение статуса заявки проведено успешно!');</script>";
    ...
    {
        echo "<script>alert('ПРОВЕРКА НА ПЛАГИАТ НЕ ЗАВЕРШЕНА!!! ВЕРНИТЕСЬ ПОЗЖЕ К ЗАДАЧЕ ДЛЯ ЗАВЕРШЕНИЯ ПРОЦЕССА!!!');</script>";
        header("Refresh: 0; url= aplications2.php");
    }   
}
...
echo <<< _begin
    ...
    <a align="center">ПРОВЕРКА НА ЗНАЧИМОСТЬ</a>
    </div>
    <div id='$id' >
        <div class='container'>
              ...
              <form action='aplications2_2_2.php' method='post'>

              <fieldset>
              <legend>По результатам анализа пакета документов примите решение:</legend> <br>
              <input type='radio' name='plagiat' value='1'>ДОКУМЕНТЫ ПРОВЕРЕНЫ - ПЛАГИАТ НЕ ПОДТВЕРЖДЕН<br><br>
              <input type='radio' name='plagiat' value='0'>ДОКУМЕНТЫ ПРОВЕРЕНЫ - ВЫЯВЛЕН ПЛАГИАТ - ЗАЯВКА ОТКЛОНЯЕТСЯ<br><br>
